...
class VLFusion(nn.Module):
    def __init__(self, transformer, pos):
        ...
        self.learnable_token = nn.Parameter(torch.randn(1, hidden_dim)) # TODO
    ...
    def forward(self, fv, fl):
        ...
        learnable_token = self.learnable_token.unsqueeze(0).expand(bs, -1, 1)  # TODO [batch size, hidden dim, 1]
        input_tokens = torch.cat([learnable_token, pv, pl], dim=2) # TODO [batch size, hidden dim, 1 + vision token length + language token length]
...
def train_epoch(train_loader, model, optimizer, epoch, args):
    ...
    for batch_idx, (imgs, masks, word_id, word_mask, gt_bbox) in enumerate(train_loader):
        ...
        pred_bbox = model(image, masks, word_id, word_mask) # TODO : Get the pred_box from the model.forward call. 
        scale = args.size-1
        ...
        original_size_pred_bbox = pred_bbox * scale # TODO: convert the normalized size to the original size
        original_size_x1y1x2y2_pred_bbox = xywh2xyxy(original_size_pred_bbox) # TODO: convert the xywh format to the x1y1x2y2 format
        ...
        xywh_gt_bbox = xyxy2xywh(gt_bbox) # TODO: convert the x1y1x2y2 format to the xywh format
        normalized_size_xywh_gt_bbox = xywh_gt_bbox / scale # TODO: convert the normalized size to theoriginal size
        ...
        xyxy_pred_bbox = xywh2xyxy(pred_bbox * scale) # TODO: convert the xywh format to the x1y1x2y2 format
        original_size_xyxy_pred_bbox = xyxy_pred_bbox # TODO: convert the normalized size to the original size
...
def bbox_iou(box1, box2, x1y1x2y2=True):
    ...
    inter_area =torch.clamp(inter_x2 - inter_x1, min=0) * torch.clamp(inter_y2 - inter_y1, min=0) # TODO: Intersection area
    ...
    union_area = area_box1 + area_box2 - inter_area # TODO: Union area
    union_area += 1e-16 # For numerical stability
    # Finally, calculate the IoU
    iou = iou = inter_area / union_area# TODO: IoU
    # Union Area
